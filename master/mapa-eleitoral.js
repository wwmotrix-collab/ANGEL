  ...
  root.innerHTML = `
    <div class="dash-view mapa-eleitoral-view">
      <div class="camp-hero">
        ...
        <div class="camp-desc">
          Visualize locais de votação, zonas, seções, eleitores conhecidos e força territorial por ponto.
        </div>
      </div>
      ...
      <section class="mapa-panel">
        ...
        <div id="mapaDetalhe" class="empty">
          <div class="empty-icon">📍</div>
          Selecione um PIN ou local para ver os detalhes.
        </div>
      </section>
...
async function carregar(){
  try{
    ...
    document.getElementById('mapaLista').innerHTML = `<div class="empty err">Erro ao carregar mapa: ${esc(e.message||e)}</div>`;
  }
}
...
function normItem(v,id){
  ...
  return {
    ...
    votosGuto: num(pick(v,['votos_guto','votosGuto'], 0)),
    pctGuto: num(pick(v,['pct_guto','pctGuto'], 0)),
...
function renderMapa(){
  ...
  el.innerHTML = `
    <div class="mapa-mini">
      <div class="mapa-mini-head">
        <strong>${esc(territorio.municipio)} / ${esc(territorio.uf)}</strong>
        <small>${comGeo.length} PINs com coordenada · ${semGeo.length} pendentes</small>
      </div>
      <div class="pin-cloud">
        ${filtrados.slice(0,160).map(pinHtml).join('')}
      </div>
    </div>
  `;
}
...
function pinHtml(l){
  ...
  return `
    <button class="pin ${cls} s${size}" title="${escAttr(l.nome)}" onclick="mapaEleitoralSelecionar('${escAttr(l.id)}')">
      <span>${pinLabel(l)}</span>
    </button>
  `;
}
...
function detalhe(){
  ...
  el.innerHTML = `
    <div class="mapa-detail">
      ...
      <div class="camp-desc">
        ${esc(l.endereco)}<br>
        ${esc(l.bairro||'Bairro não informado')}
      </div>
      ...
      <div class="mapa-detail-grid">
        ...
        <div><strong>${fmt(l.votosGuto)}</strong><span>Votos Guto</span></div>
        <div><strong>${l.pctGuto ? l.pctGuto + '%' : '—'}</strong><span>% Guto</span></div>
      </div>

      <div class="mapa-coords">
        <code>lat: ${l.lat ?? 'pendente'}</code>
        <code>lng: ${l.lng ?? 'pendente'}</code>
      </div>

      <div class="camp-desc">
        Próxima etapa: #12 poderá marcar prioridade, meta, rota e responsável por este local.
      </div>
    </div>
  `;
}
...
function temEleitores(l){
  return Number.isFinite(l.eleitores) && l.eleitores > 0;
}

function eleitoresLabel(l){
  return temEleitores(l) ? `${fmt(l.eleitores)} eleitores` : 'eleitores pendentes';
}

function pinSize(l){
  if(!temEleitores(l)) return 1;
  if(l.eleitores >= 9000) return 5;
  if(l.eleitores >= 6000) return 4;
  if(l.eleitores >= 3000) return 3;
  if(l.eleitores >= 1000) return 2;
  return 1;
}

function pinLabel(l){
  if(temEleitores(l)) return Math.round(l.eleitores/1000)+'k';
  return '?';
}
...
function style(){
  ...
  s.textContent = `
    .mapa-source-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:14px;margin-bottom:14px;color:var(--muted);font-size:13px}
    .mapa-source-card div{margin-bottom:6px}.mapa-source-card strong{color:var(--text)}.mapa-source-card code{display:block;color:var(--muted);font-size:11px;overflow:auto}
    .mapa-panel{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:14px;margin-bottom:14px}
    .mapa-filters{display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:10px}
    .mapa-canvas{min-height:360px}
    .mapa-mini{border:1px solid var(--border);border-radius:16px;padding:14px;background:radial-gradient(circle at 20% 10%,rgba(132,255,0,.12),transparent 30%),radial-gradient(circle at 80% 80%,rgba(59,130,246,.12),transparent 35%),var(--bg)}
    .mapa-mini-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:14px}.mapa-mini-head small{color:var(--muted)}
    .pin-cloud{display:flex;flex-wrap:wrap;gap:9px;align-content:flex-start;min-height:300px}
    .pin{border:1px solid var(--border);border-radius:999px;background:rgba(132,255,0,.14);color:var(--text);font-weight:800;display:flex;align-items:center;justify-content:center}
    .pin.ok{box-shadow:0 0 0 1px rgba(132,255,0,.25)}.pin.pending{background:rgba(245,158,11,.15)}
    .pin.s1{width:30px;height:30px;font-size:10px}.pin.s2{width:36px;height:36px;font-size:11px}.pin.s3{width:44px;height:44px;font-size:12px}.pin.s4{width:54px;height:54px;font-size:13px}.pin.s5{width:66px;height:66px;font-size:14px}
    .mapa-list{display:flex;flex-direction:column;gap:8px;max-height:520px;overflow:auto}
    .mapa-item{width:100%;text-align:left;background:var(--surface2);border:1px solid var(--border);border-radius:12px;padding:12px;display:flex;justify-content:space-between;gap:10px;color:var(--text)}
    .mapa-item strong{display:block;font-size:14px}.mapa-item small{display:block;color:var(--muted);font-size:11px;margin-top:4px}
    .mapa-badge{font-size:10px;border-radius:999px;padding:5px 8px;height:max-content}.mapa-badge.ok{background:rgba(34,197,94,.18);color:#22c55e}.mapa-badge.warn{background:rgba(245,158,11,.18);color:#f59e0b}
    .mapa-detail-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:14px 0}
    .mapa-detail-grid div{background:var(--surface2);border:1px solid var(--border);border-radius:12px;padding:12px}.mapa-detail-grid strong{display:block;font-size:18px}.mapa-detail-grid span{display:block;color:var(--muted);font-size:11px;margin-top:4px}
    .mapa-coords{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}.mapa-coords code{background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:8px;color:var(--muted)}
    @media(max-width:760px){.mapa-filters{grid-template-columns:1fr}.mapa-mini-head{align-items:flex-start;flex-direction:column}.mapa-detail-grid{grid-template-columns:1fr 1fr}}
  `;
