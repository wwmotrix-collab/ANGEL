...
(function (global) {
      ...
      <div class="dash-view" style="padding-bottom:110px;">
        ...
        <div id="modalRota" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:900;background:var(--surface);border-top:2px solid var(--accent);border-radius:20px 20px 0 0;padding:20px;max-height:78vh;overflow:auto;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
            ...
            <button id="btnFecharModalRota" class="btn btn-ghost" style="width:auto;padding:8px 12px;">✕</button>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px;">
            <button id="btnLimparPontos" class="btn btn-ghost">Limpar pontos</button>
            <button id="btnCancelarRota" class="btn btn-ghost">Cancelar</button>
            <button id="btnSalvarRota" class="btn btn-primary" style="grid-column:1/-1;">Salvar Rota</button>
          </div>
        </div>
      </div>
    `;
  ...
  function _inicializarMapa() {
    if (!global.L) {
      document.getElementById('rotasMap').innerHTML = '<div class="empty">Leaflet não carregado</div>';
      return;
    }
    if (_map) return;
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(pos => {
        ...
        _map.setView(userPos, 14);
        L.circleMarker(userPos, { radius: 6 }).addTo(_map).bindPopup('Você está aqui');
      }, () => {});
    }
  ...
  function _assinar() {
    _unsubRotas = WWMX.db.on(`campanhas/${_campanhaId}/rotas`, (snap) => {
      const rotas = Object.values(snap.val() || {});
      _renderizarLista(rotas);
      _renderizarStats(rotas);
      _renderizarRotasNoMapa(rotas);
    });
    _unsubMilitantes = WWMX.db.on(`campanhas/${_campanhaId}/militantes`, (snap) => {
      ...
      if (select) select.innerHTML = '<option value="">Selecione...</option>' + mils.map(m => `<option value="${m.uid}">${_esc(m.nome || 'Sem nome')}</option>`).join('');
    });
  }
  ...
  function _renderizarLista(rotas) {
    ...
    lista.innerHTML = rotas.sort((a,b) => (b.ts || 0) - (a.ts || 0)).map(r => `
      <div class="banner-item">
        <div style="font-size:24px;">${_iconTipo(r.tipo)}</div>
        <div class="banner-info">
          <div class="banner-name">${_esc(r.nome || r.titulo || 'Rota sem nome')}</div>
          <div class="banner-meta">${Number(r.km || 0).toFixed(1)} km · ${(r.pontos || []).length} pontos · ${_esc(r.motoristaNome || r.responsavel || 'sem responsável')}</div>
        </div>
        <div style="display:flex;gap:6px;">
          <button onclick="window.dispatchEvent(new CustomEvent('wwmx:focar-rota', { detail: { rotaId: '${r.id}' } }))" class="btn btn-ghost" style="padding:6px;">🗺️</button>
  ...
  function _renderizarStats(rotas) {
    ...
    stats.innerHTML = `
      <div class="dash-stats">
        ...
        <div class="stat-card ret"><div class="stat-num red">${km.toFixed(1)}</div><div class="stat-label">Km</div></div>
      </div>
    `;
  }
  ...
  function _renderizarRotasNoMapa(rotas) {
    ...
    _map._rotaLayers = [];
    rotas.forEach(r => {
      const pts = (r.pontos || []).map(p => [p.lat, p.lng]).filter(p => Number.isFinite(p[0]) && Number.isFinite(p[1]));
      if (pts.length < 2) return;
      const line = L.polyline(pts, { weight: 4 }).addTo(_map).bindPopup(_esc(r.nome || 'Rota'));
      _map._rotaLayers.push(line);
    });
  }

  function _ativarModoRota() {
    _inicializarMapa();
    if (!_map) { WWMX.UI?.showToast?.('Mapa indisponível', 'error'); return; }
    _modoRota = true;
    _limparPontos();
    _polylineTemp = L.polyline([], { weight: 5, dashArray: '8,4' }).addTo(_map);
    _map.on('click', _onMapClick);
    const modal = document.getElementById('modalRota');
    if (modal) modal.style.display = 'block';
    document.getElementById('rotaNome').value = '';
    WWMX.UI?.showToast?.('Clique no mapa para adicionar pontos da rota');
    setTimeout(() => _map.invalidateSize(), 250);
  }
  ...
  function _onMapClick(e) {
    ...
    _pontos.push(ponto);
    const marker = L.marker([ponto.lat, ponto.lng]).addTo(_map).bindPopup(`Ponto ${_pontos.length}`);
    _markersTemp.push(marker);
    if (_polylineTemp) _polylineTemp.setLatLngs(_pontos.map(p => [p.lat, p.lng]));
    _atualizarPontosInfo();
  }

  function _atualizarPontosInfo() {
    const info = document.getElementById('rotaPontosInfo');
    if (!info) return;
    if (!_pontos.length) { info.textContent = 'Clique no mapa para adicionar pontos.'; return; }
    const km = _calcularKm(_pontos);
    info.innerHTML = `<strong>${_pontos.length} ponto(s) · ${km.toFixed(2)} km estimados</strong>` + _pontos.map((p,i) => `<div>${i+1}: ${p.lat.toFixed(5)}, ${p.lng.toFixed(5)}</div>`).join('');
  }
  ...
  async function _salvarRota() {
    ...
    const select = document.getElementById('rotaMotorista');
    const motoristaUid = select.value;
    const motoristaNome = select.options[select.selectedIndex]?.text || '';
    ...
    const rota = { id: Date.now().toString(), nome, tipo, pontos: _pontos, km: _calcularKm(_pontos), motoristaUid, motoristaNome, status: 'planejada', criadoPor: session.nome || 'coord', ts: Date.now() };
    await WWMX.db.set(`campanhas/${_campanhaId}/rotas/${rota.id}`, rota);
    WWMX.UI?.showToast?.(`✅ Rota "${nome}" salva!`, 'success');
    _desativarModoRota();
  }

  function _limparPontos() {
    _pontos = [];
    _markersTemp.forEach(m => m.remove());
    _markersTemp = [];
  ...
  function _desativarModoRota() {
    if (_map) _map.off('click', _onMapClick);
    _modoRota = false;
    _limparPontos();
    const modal = document.getElementById('modalRota');
  ...
  global.addEventListener('wwmx:focar-rota', async (e) => {
    const rotaId = e.detail.rotaId;
    const snap = await WWMX.db.get(`campanhas/${_campanhaId}/rotas/${rotaId}`);
    const rota = snap.val?.() || snap;
    const pts = (rota?.pontos || []).map(p => [p.lat, p.lng]);
